...
- [ ] **Clerk 인증 설정**
  ...
  - [ ] Clerk 프로젝트 생성 및 환경변수 설정
  - [ ] 한국어 로컬라이제이션 적용
  - [ ] 로그인/회원가입 페이지 구현 (`/app/sign-in`, `/app/sign-up`)
  - [ ] 소매점 사용자 역할(role) 구분 로직 추가
- [ ] **초기 리다이렉트 구현 (R.AUTH.02)**

  - [ ] 로그인 성공 시 `/retailer/dashboard` 자동 이동
  - [ ] 미인증 사용자 접근 시 로그인 페이지로 리다이렉트
  - [ ] `middleware.ts`에서 경로 보호 설정
...
- [ ] **공통 레이아웃 구조**
  ...
  - [ ] `/app/retailer/layout.tsx` 생성
  - [ ] 헤더 컴포넌트 (`components/retailer/header.tsx`)
    - 로고
    - 사용자 메뉴
    - 로그아웃 버튼
...
- [ ] **최근 주문 요약 (R.DASH.03)**
  ...
  - [ ] 최근 주문 3건 타임라인 컴포넌트 (`components/retailer/recent-orders-timeline.tsx`)
  - [ ] 주문 상태별 색상/아이콘 표시
  - [ ] 배송 상태 표시 (새벽 배송 준비 중, 일반 배송 중)
  - [ ] 상세 페이지로 이동하는 링크
- [ ] **긴급 알림/공지 (R.DASH.04)**

  - [ ] 알림 배너 컴포넌트 (`components/retailer/alert-banner.tsx`)
  - [ ] 재고 부족 알림 표시
  - [ ] 플랫폼 공지사항 표시
  - [ ] 다크 모드 최적화
...
- [ ] **데이터베이스 스키마**
  ...
  - [ ] `products` 테이블 마이그레이션 생성
    - AI 표준화 상품명 필드 (`standardized_name`)
    - 원본 상품명 필드 (`original_name`)
    - 카테고리 필드 (`category`: 엽채류, 근채류 등)
    - 가격 필드 (`price`)
    - 재고 필드 (`stock`)
    - 배송 옵션 필드 (`delivery_options`)
    - 도매상 ID 필드 (`wholesaler_id`)
...
- [ ] **Smart Search - Command Palette (R.SEARCH.01)**
  ...
  - [ ] 주문 내역 검색 기능
  - [ ] AI 질의 모드 통합 (Phase 5)
- [ ] **상품 리스트 페이지 (R.SEARCH.02, R.SEARCH.03)**

  - [ ] `/app/retailer/products/page.tsx` 생성
  - [ ] 상품 카드 컴포넌트 (`components/retailer/product-card.tsx`)
    - AI 표준화 상품명 표시
    - 익명 판매자 정보 (Partner #XXX)
    - 가격 표시
    - 배송 옵션 배지 (새벽 배송 가능)
  - [ ] 필터링 UI (`components/retailer/product-filters.tsx`)
    - 카테고리 필터
    - 새벽 배송 가능 필터 (R.SEARCH.03)
    - 가격 범위 필터
  - [ ] 정렬 옵션 (가격순, 인기순)
  - [ ] 무한 스크롤 또는 페이지네이션
  - [ ] React Query를 사용한 데이터 페칭 및 캐싱
- [ ] **상품 상세 페이지 (R.SEARCH.04, R.SEARCH.05)**

  - [ ] `/app/retailer/products/[id]/page.tsx` 생성
  - [ ] 상품 이미지 갤러리 (Next.js Image 컴포넌트 사용)
  - [ ] AI 표준화 상품명 & 규격 표시
  - [ ] 익명 판매자 정보 표시
  ...
  - [ ] 배송 옵션 정보 표시
    ...
    - 배송 가능 시간대
  - [ ] 수량 선택 UI (Kg, Box 단위)
  - [ ] 장바구니 담기 버튼 (R.SEARCH.05)
- [ ] **도매 정보 익명화 (R.SEARCH.04)**
...
- [ ] **장바구니 상태 관리 (R.CART.01)**

  - [ ] Zustand 스토어 생성 (`stores/cart-store.ts`)
    - 장바구니 아이템 배열
    - 총 금액 계산
  - [ ] 장바구니 추가 액션 (`addToCart`)
  - [ ] 장바구니 수정 액션 (`updateCartItem`)
  - [ ] 장바구니 삭제 액션 (`removeFromCart`)
  - [ ] 로컬 스토리지 연동 (페이지 새로고침 대응)
- [ ] **장바구니 페이지 (R.CART.02, R.CART.03)**

  - [ ] `/app/retailer/cart/page.tsx` 생성
  - [ ] 상품 목록 테이블/카드 뷰
    - 상품 이미지
    - AI 표준화 상품명
    - 익명 판매자
    - 단가 및 수량
    - 소계
  - [ ] 수량 증가/감소 버튼 (R.CART.02)
  - [ ] 개별 상품 삭제 버튼 (R.CART.02)
  - [ ] 전체 삭제 버튼
  - [ ] 빈 장바구니 상태 UI
- [ ] **가격 계산 로직 (R.CART.03)**
...
- [ ] **데이터베이스 스키마**
  ...
  - [ ] `order_items` 테이블 마이그레이션
    ...
    - 단가 (`unit_price`)
    - 소계 (`subtotal`)
- [ ] **주문서 페이지 (R.ORDER.01, R.ORDER.02)**

  - [ ] `/app/retailer/checkout/page.tsx` 생성
  - [ ] 주문 상품 목록 요약
    - 상품명, 수량, 가격
  - [ ] 배송지 정보 입력 폼
    - 주소 검색 API 연동 (선택사항)
...
- [ ] **주문 목록 페이지 (R.MY.01)**
  ...
  - [ ] `/app/retailer/orders/page.tsx` 생성
  - [ ] 주문 목록 테이블/카드 뷰
    - 주문 번호
    - 주문 날짜
    - 상품명 (대표 상품 + N개)
    - 주문 상태
    - 총 금액
  - [ ] 상태별 필터 (전체, 접수, 배송 중, 완료, 취소)
  - [ ] 날짜 범위 필터
  - [ ] 검색 기능 (주문 번호, 상품명)
  - [ ] 페이지네이션
  - [ ] React Query를 사용한 데이터 페칭
- [ ] **주문 상세 페이지 (R.MY.01, R.MY.02)**

  - [ ] `/app/retailer/orders/[id]/page.tsx` 생성
  - [ ] 주문 정보 요약
    - 주문 번호
    - 주문 날짜
    - 주문 상태
...
- [ ] **배송 타임라인 UI (R.MY.02)**
  ...
  - [ ] 타임라인 컴포넌트 구현 (`components/retailer/delivery-timeline.tsx`)
  - [ ] 주문 접수 → 준비 → 배송 중 → 완료 단계 표시
  - [ ] 선택한 배송 예정 시간 강조
  - [ ] 각 단계별 타임스탬프 표시
  - [ ] 현재 진행 중인 단계 하이라이트
  - [ ] 완료된 단계는 체크 표시
